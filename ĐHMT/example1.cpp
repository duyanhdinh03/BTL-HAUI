...
// Khai báo các tham số chiếu sáng - tô bóng //
...
void keyboard(unsigned char key, int x, int y)
{
	if (duyanhControl == false || phucControl == false || datControl == false || honganhControl == false) {
		...
		case '+':
			...
			break;
		// Chỉnh góc nhìn 
		case 'p':  // Chuyển mắt nhìn về vị trí ban đầu đứng
			eye = vec4(0, 3, -30, 1);
			matXY[0] = 0; 
			at = vec4(0, 0, 0, 1);
			mat_dinhhuong = vec4(0, 0, 1, 1);
			break;
		case '`': // Lùi về sau 100 đơn vị
			eye -= 100 * normalize(mat_dinhhuong);
			break;
		case '~': // Chuyển sang góc nhìn flycam và nghiêng 45 độ
			eye = vec4(0, 240, -130, 1); // Điểm nhìn từ trên xuống và dịch trái 30 đơn vị
			at = vec4(0, 0, 0, 1); // Điểm nhìn về gốc tọa độ
			matXY[0] = 60; // Góc nghiêng 45 độ
			mat_dinhhuong = RotateZ(matXY[0]) * mat_dinhhuong; // Quay mắt nhìn theo trục Z
			break;
		case '-'  : 
			eye -= 30 * normalize(mat_dinhhuong); // Lùi về sau 30 đơn vị
			eye += 40 * normalize(cross(mat_dinhhuong, up)); // Dịch phải 40 đơn vị
			at = eye - 30 * normalize(mat_dinhhuong); // Điểm nhìn dịch sau 30 đơn vị
			vec4 right = normalize(cross(mat_dinhhuong, up));
			matXY[1] = 90;
			mat_dinhhuong = RotateY(matXY[1]) * mat_dinhhuong; 
			break;
	...
	if (honganhControl == true) {
		...
		case '2':
			di_chuyen_demZ -= 0.15;
			if (di_chuyen_demZ < -2.4) di_chuyen_demZ = -2.4, di_chuyen_demY = -0.81, xoay_demX = 0;
			glutPostRedisplay();
			break;
		case '3':
			xoay_demY += 5;
